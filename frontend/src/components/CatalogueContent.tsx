...
import { ChevronLeft, ChevronRight, ChevronDown, ChevronRight as ChevronRightIcon, SlidersHorizontal, X } from "lucide-react";
...
</aside>{filtersOpen && <div className="fixed inset-0 bg-black/60 z-40 md:hidden" onClick={() => setFiltersOpen(false)} />}<div className="min-w-0 flex-grow"><div className="flex justify-between items-end gap-4 mb-6"><div className="min-w-0"><h1 className="break-words text-3xl md:text-5xl font-black tracking-tighter uppercase italic mb-2">{getTitle()}</h1><p className="text-gray-500 text-xs uppercase tracking-widest">{loading ? "Chargement..." : `${total} produit${total !== 1 ? "s" : ""}`}</p></div><button onClick={() => setFiltersOpen(true)} className="md:hidden flex shrink-0 items-center gap-2 text-xs font-black tracking-widest uppercase text-white border border-white/10 px-4 py-3 hover:bg-white/5"><SlidersHorizontal size={14}/> FILTRER</button></div>{activeBadges.length > 0 && <div className="mb-8 flex flex-wrap gap-2">{activeBadges.map((badge) => <button key={badge.key} onClick={badge.remove} className="inline-flex items-center gap-2 border border-[#ff4a8d]/40 bg-[#ff4a8d]/10 px-3 py-1.5 text-xs font-bold text-pink-100">{badge.label} <X size={12}/></button>)}</div>}{loading ? <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="rounded-xl border border-gray-100 bg-white aspect-[4/5] animate-pulse" />)}</div> : products.length === 0 ? <div className="text-center py-32 border border-white/5"><p className="text-gray-500 uppercase tracking-widest text-xs mb-8">Aucun produit trouvé</p><button onClick={clearFilters} className="bg-[#ff4a8d] text-white px-8 py-4 text-xs font-black tracking-widest uppercase hover:bg-[#ff1f70]">EFFACER LES FILTRES</button></div> : <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div>}{totalPages > 1 && <div className="flex items-center justify-center gap-3 mt-16"><button onClick={() => updateParams("page", String(page - 1))} disabled={page <= 1} className="w-10 h-10 border border-white/10 flex items-center justify-center hover:bg-white/5 disabled:opacity-20"><ChevronLeft size={16}/></button>{Array.from({ length: Math.min(totalPages, 7) }, (_, i) => i + 1).map((p) => <button key={p} onClick={() => updateParams("page", String(p))} className={`w-10 h-10 text-xs font-black tracking-widest ${p === page ? "bg-[#ff4a8d] text-white" : "border border-white/10 text-gray-400 hover:bg-white/5"}`}>{p}</button>)}{totalPages > 7 && <span className="text-gray-600 text-xs">...</span>}<button onClick={() => updateParams("page", String(page + 1))} disabled={page >= totalPages} className="w-10 h-10 border border-white/10 flex items-center justify-center hover:bg-white/5 disabled:opacity-20"><ChevronRight size={16}/></button></div>}</div></div></div>;
}
