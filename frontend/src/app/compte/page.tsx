...
function AddressesPanel({ addresses, setAddresses, setMessage }: { addresses: CustomerAddress[]; setAddresses: (addresses: CustomerAddress[]) => void; setMessage: (message: string) => void }) {
  ...
  const handleAdd = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!form.firstName.trim() || !form.lastName.trim() || !form.address.trim() || !form.postalCode.trim() || !form.city.trim() || !form.country.trim()) {
    ...
    try {
      ...
      setMessage(err instanceof Error ? err.message : "Erreur lors de l’ajout de l’adresse.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        <PanelTitle title="Mes adresses" subtitle="Ajoutez ou supprimez vos adresses enregistrées." />
        <button type="button" onClick={() => setShowForm((value) => !value)} className="bg-[#E91E8C] px-5 py-3 text-[11px] font-black uppercase tracking-[0.18em] text-white hover:bg-[#ff4a9f]">Ajouter une adresse</button>
      </div>

      {showForm && (
        <form onSubmit={handleAdd} className="mt-8 grid gap-5 border border-[#E91E8C]/25 bg-[#E91E8C]/5 p-5 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2 flex gap-3">
            <button type="submit" disabled={saving} className="bg-[#E91E8C] px-6 py-3 text-[11px] font-black uppercase tracking-[0.2em] text-white disabled:opacity-60">{saving ? "Ajout..." : "Enregistrer l'adresse"}</button>
            <button type="button" onClick={() => setShowForm(false)} className="border border-white/10 px-6 py-3 text-[11px] font-black uppercase tracking-[0.2em] text-white/60 hover:text-white">Annuler</button>
          </div>
        </form>
      )}
        ...
        {addresses.length === 0 ? <EmptyState text="Aucune adresse enregistrée." /> : addresses.map((item) => (
          <article key={item.id} className="border border-white/10 bg-black/20 p-5">
            <div className="flex justify-between gap-4">
              <div>
                <h3 className="font-black uppercase">{item.firstName} {item.lastName}</h3>
                <p className="mt-3 text-sm leading-6 text-white/55">{item.address}<br />{item.postalCode} {item.city}<br />{item.country}</p>
                {item.phone && <p className="mt-2 text-sm text-white/45">{item.phone}</p>}
              </div>
              <button type="button" onClick={() => handleDelete(item.id)} className="h-fit text-red-300 hover:text-red-200" aria-label="Supprimer l’adresse"><Trash2 size={18} /></button>
            </div>
          </article>
        ))}
      </div>
          ...
          <article key={product.id} className="overflow-hidden border border-white/10 bg-black/20">
            ...
            <div className="p-4">
              ...
              <div className="mt-4 grid gap-2">
                <Link href={`/produits/${product.slug}`} className="bg-white px-4 py-3 text-center text-[10px] font-black uppercase tracking-[0.18em] text-black hover:bg-[#E91E8C] hover:text-white">Voir le produit</Link>
