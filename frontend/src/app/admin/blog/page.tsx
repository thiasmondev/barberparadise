  ...
  const requestVisualSuggestions = async (source: "ai" | "pexels") => {
    ...
    try {
      ...
      if (suggestions.length === 0) {
        throw new Error(source === "ai"
          ? "La génération IA n’a retourné aucun visuel. Réessayez dans quelques instants."
          : "Aucune image libre de droit n’a été trouvée pour cette section. Modifiez le texte ou essayez une autre source.");
      }
      setVisualSuggestions(suggestions);
      setVisualAlt(suggestions[0]?.altText || result.brief.altText);
